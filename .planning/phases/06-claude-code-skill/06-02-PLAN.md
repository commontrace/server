...
2. Write a session-scoped flag file to `/tmp/commontrace-prompted-{pid}` after first prompt — check it before prompting again (prevents prompting on every Claude response in the same session)
...
2. **Loop prevention (CRITICAL — first two checks):**
   ...
   - Check for session-scoped flag file at `/tmp/commontrace-prompted-{pid}` where pid is `os.getpid()` of the parent process (or use `session_id` from stdin if available, fall back to `os.getppid()`). If file exists, exit 0 (already prompted this session)
...
4. **Contribution prompt:**
   - Write the flag file to prevent re-prompting: create `/tmp/commontrace-prompted-{id}` with current timestamp
   - Output JSON to stdout:
     ```json
     {
       "decision": "block",
