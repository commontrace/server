...
The actual parameter type is `RequireEmail`, not `CurrentUser`. The docstring was written before Phase 4 changed the dependency.
  ...
  <verify>
Run: `cd /home/bitnami/commontrace && python -c "from api.app.services.tags import normalize_tag, validate_tag; print('normalize_tag and validate_tag importable')"` — should succeed.
Run: `cd /home/bitnami/commontrace && python -c "from api.app.services.tags import normalize_tags" 2>&1` — should fail with ImportError (function removed).
