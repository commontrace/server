...
- Add `from app.routers import amendments, auth, moderation, reputation, search, tags, traces, votes` to the imports
...
- Output: `"Trace submitted successfully (ID: uuid). Status: pending — it will be validated after community review."`
...
Note: `service_started` not `service_healthy` -- the API doesn't need to be healthy for the MCP server to start (circuit breaker handles backend unavailability). But it DOES need to be started so Docker networking resolves the `api` hostname.
  ...
  <verify>
    Run: `cd /home/bitnami/commontrace/mcp-server && uv run python -c "from app.server import mcp; print(mcp.name)"` -- prints "CommonTrace".
    Run: `grep -c '@mcp.tool' /home/bitnami/commontrace/mcp-server/app/server.py` -- prints 5 (one decorator per tool).
...
<verification>
1. `grep -c '@mcp.tool' /home/bitnami/commontrace/mcp-server/app/server.py` == 5
