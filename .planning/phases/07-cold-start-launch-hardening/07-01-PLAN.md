...
4. For each trace:
   ...
   c. **IMPORTANT field mapping:** JSON uses short names, ORM uses `_text` suffix — map `context_text=trace_json['context']`, `solution_text=trace_json['solution']` (see existing `api/fixtures/seed_fixtures.py` lines 97-98 for the pattern)
   d. Process tags: normalize with `normalize_tag()`, validate with `validate_tag()`, get-or-create Tag rows, insert into `trace_tags` join table
